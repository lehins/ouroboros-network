...
instance ApplyBlock (LedgerState BlockA) BlockA where
  applyLedgerBlock cfg blk =
        fmap setTip
      . repeatedlyM
          (fmap fst .: applyTx cfg DoNotForgive (blockSlot blk))
          (blkA_body blk)
    where
...
instance LedgerSupportsMempool BlockA where
  applyTx _ _wtf sno tx@(TxA _ payload) (TickedLedgerStateA st) =
      case payload of
        InitiateAtoB -> do
          return (TickedLedgerStateA $ st { lgrA_transition = Just sno }, ValidatedGenTxA tx)

  reapplyTx cfg slot = fmap fst .: (applyTx cfg DoNotForgive slot . forgetValidatedGenTxA)
